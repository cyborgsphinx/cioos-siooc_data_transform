...
def write_ctd_ncfile(filename, ctdcls, config={}):
    ...
    global_attrs["time_coverage_start"] = ctdcls.file.start_time.strftime(
        date_format
    )
    global_attrs["time_coverage_end"] = ctdcls.file.start_time.strftime(
        date_format
    )
    ...
    for i, channel in enumerate(ctdcls.file.channels):
        ...
        #     Nutrients in bottle files
            ...
            try:
                ...
                print(e)
        #  Chlorophyll
        elif (is_in(["chlorophyll:extracted", "fluorescence", "transmissivity", "ammonium", "speed:sound", "ph:"], channel.name) or channel.name.lower() == "ph") and not is_in(
            ["flag"], channel.name
        ):
            try:
                ncfile.add_var(
